def validPath(n: int, edges, source: int, destination: int) -> bool:
    if source == destination:
        return True
    if not edges or len(edges) <= 0 or n < 0:
        return False
    ...
    for i in range(len(edges)):
        l = edges[i]
        if source in l:
            new_edges = [e for e in edges if source not in e]
            new_source = l[0] if l[1] == source else l[1]
            if new_source == destination:
                return True
            print(f"Trying path for source {new_source} to destination {destination} with n={n-1}")
            if validPath(n-1, new_edges, new_source, destination):
                return True
...
#   destination = 2
#   n = 3
#   print(f"Trying path for source {source} to destination {destination} with n={n}")
