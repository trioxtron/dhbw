...
def main():
    ...
    end_time = time.time()
    print(f"Majority element (fast): {result_fast}, Time taken: {end_time - start_time:.6f} seconds")

    # Measure time for the slow function
    start_time = time.time()
    result_slow = majority_element(nums)
    end_time = time.time()
    print(f"Majority element (slow): {result_slow}, Time taken: {end_time - start_time:.6f} seconds")
